...
# 创建表
CREATE TABLE `user` (
	id BIGINT PRIMARY KEY COMMENT '关键字',
	username VARCHAR(10) UNIQUE COMMENT '用户名',
	`password` VARCHAR(20) COMMENT '登录密码',
	contact_id BIGINT COMMENT '外键，关联联系人，表示自己的信息',
...
CREATE TABLE `group` (
	...
	user_id BIGINT COMMENT '外键，关联用户',
	is_deleted TINYINT(1) COMMENT '逻辑删除',
...
CREATE TABLE contact (
	...
	user_id BIGINT COMMENT '外键，关联用户',
	group_id BIGINT COMMENT '外键，关联分组',
	is_deleted TINYINT(1) COMMENT '逻辑删除',
	FOREIGN KEY (user_id) REFERENCES `user`(id),
...
CREATE TABLE mail (
	...
	contact_id BIGINT COMMENT '外键，关联联系人',
	is_deleted TINYINT(1) COMMENT '逻辑删除',
